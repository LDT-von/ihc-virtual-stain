...
class MarkerContextTests(unittest.TestCase):
    ...
    # 本快照移除了 test_legacy_tta_restores_coordinates_and_uses_eight_calls：
    # 它校验的是初赛阶段的 8x TTA（inference_8x_tta / tta_eval，走 Pix2Pix 的
    # model.generator(x, condition) 接口），与本次提交的模型无关，故未随快照复制。
    # 本模型的 4x TTA 覆盖在 test_tta_inverse_is_exact_and_unique 中。
