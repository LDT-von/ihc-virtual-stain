...
策略：用 EMA (Exponential Moving Average) 权重平滑 G 的参数
通常带来 +0.005~0.015 SSIM 提升
"""
import argparse
import sys
import time
...
def parse_args():
    ...
    p.add_argument('--save-prefix', default='pix2pix_v9b')
...
def main():
    ...
    print('=' * 60)
    print(f'[v9b EMA+PureImg] marker={args.marker} epochs={args.epochs} lr={args.lr}')
    print(f'[v9b] resume={args.resume} ema_decay={args.ema_decay}')
    print('=' * 60)
    ...
    for epoch in range(args.epochs):
        ...
        print(f'[Epoch {epoch}] G={avg_g:.3f} ema_ssim={ssim_v:.4f} (raw={eval_skim(model, val_loader, device):.4f}) t={elapsed:.0f}s')
        ...
        if ssim_v > best_ssim:
            ...
            print(f'  [Best EMA] epoch={epoch} val_ssim={ssim_v:.4f}')

    print(f'\n[Done] Initial {initial_ssim:.4f} -> Best EMA {best_ssim:.4f} (epoch {best_epoch})')
