...
class NAFBlock(nn.Module):
    def __init__(self, c):
        ...
        self.ffn_fc1 = nn.Linear(c, c * 2)
        self.ffn_fc2 = nn.Linear(c * 2, c)
        self.res = nn.Parameter(torch.ones(1, c, 1, 1) * 0.1)

    def forward(self, x):
        b, c, h, w = x.shape
        n = self.norm(x).reshape(b * h * w, c)
        ffn_out = self.ffn_fc2(SimpleGate()(self.ffn_fc1(n)))
        out = ffn_out.reshape(b, h, w, c).permute(0, 3, 1, 2)
        return x + out * self.res
