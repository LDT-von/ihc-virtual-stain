...
def train(args):
    ...
    if args.resume and Path(args.resume).resolve().parent != output.resolve() and not getattr(args, 'extend_epochs', False):
        raise ValueError('--resume must use the original output directory; use --init-checkpoint for a new run')
    if output.exists() and any(output.iterdir()) and not (args.resume and getattr(args, 'extend_epochs', False)):
    ...
    # Adaptive per-marker loss balancing: tracks EMA of per-marker L1 and inverts
    # it so harder markers receive proportionally larger gradients.
    n_markers = len(MARKERS)
    adaptive_enabled = bool(getattr(args, 'adaptive_marker_weights', True))
    adaptive_alpha = float(getattr(args, 'adaptive_alpha', 0.95))
    adaptive_eps = float(getattr(args, 'adaptive_eps', 1e-3))
    marker_ema = torch.ones(n_markers)
    if init_checkpoint:
        ...
        _stats = initial.get('marker_loss_ema')
        if _stats is not None:
            t_ema = torch.as_tensor(_stats, dtype=torch.float32)
            if t_ema.shape == marker_ema.shape:
                marker_ema = t_ema
    if args.resume:
        ...
        # Allow --epochs extension when explicitly requested via --extend-epochs flag.
        if not getattr(args, 'extend_epochs', False):
            for key in ('epochs', 'lr', 'batch_size', 'seed'):
                if ckpt['run']['args'][key] != getattr(args, key):
                    raise ValueError(f'Resume must preserve {key}')
        elif getattr(args, 'epochs', 0) <= ckpt['run']['args']['epochs']:
            raise ValueError(f'--extend-epochs requires new --epochs > {ckpt["run"]["args"]["epochs"]}')
        if ckpt['run']['environment']['source_sha256'] != run['environment']['source_sha256'] and not getattr(args, 'extend_epochs', False):
        ...
        # Re-tune LR for the extended schedule; keep momentum/state from optimizer when compatible.
        if getattr(args, 'extend_epochs', False):
            for group in optimizer.param_groups:
                group['lr'] = args.lr
        scaler.load_state_dict(ckpt['scaler'])
        start, steps, best = ckpt['epoch']+1, ckpt['steps'], ckpt['best_ssim']
        _stats_r = ckpt.get('marker_loss_ema')
        if _stats_r is not None:
            t_ema = torch.as_tensor(_stats_r, dtype=torch.float32)
            if t_ema.shape == marker_ema.shape:
                marker_ema = t_ema
    ...
    for epoch in range(start, args.epochs):
        ...
        for batch_index, (x, y, _) in enumerate(train_loader):
            ...
            with amp_context(device):
                pred = model(x)
            if adaptive_enabled:
                # Inverse-loss weighting: harder markers get more gradient. EMA stabilises.
                inv = 1.0 / (marker_ema + adaptive_eps)
                marker_weights = inv * (n_markers / inv.sum().clamp_min(1e-6))
                with torch.no_grad():
                    per = (pred - y).abs().mean(dim=(0, 2, 3)).detach().cpu()
                    marker_ema = adaptive_alpha * marker_ema + (1 - adaptive_alpha) * per
                loss = reconstruction_loss(pred, y, marker_weights=marker_weights.to(pred.device))
            else:
        ...
        state = {'format': 1, 'run': run, 'epoch': epoch, 'steps': steps, 'best_ssim': best,
                 ...
                 'marker_loss_ema': marker_ema.tolist() if adaptive_enabled else None,
...
def main():
    ...
    for name in ('train', 'eval', 'infer'):
        ...
        if name == 'train':
            ...
            p.add_argument('--extend-epochs', action='store_true',
                           help='Allow extending --epochs beyond the checkpoint value when using --resume')
            ...
            p.add_argument('--adaptive-marker-weights', dest='adaptive_marker_weights',
                           action=argparse.BooleanOptionalAction, default=True,
                           help='Per-marker inverse-L1-EMA loss balancing (default on)')
            p.add_argument('--adaptive-alpha', type=float, default=0.95)
            p.add_argument('--adaptive-eps', type=float, default=1e-3)
        else:
